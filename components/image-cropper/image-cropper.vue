<template>
	<view class='image-cropper' @touchmove='_preventTouchMove'>
		<view class='main' @touchend="_cutTouchEnd" @touchstart="_cutTouchStart" @touchmove="_cutTouchMove" @tap="_click">
			<view class='content'>
				<view class='content_top bg_gray {{flag_bright?"":"bg_black"}}' style="height:{{cut_top}}px;transition-property:{{cut_animation?'':'background'}}"></view>
				<view class='content_middle' style="height:{{height}}px;">
					<view class='content_middle_left bg_gray {{flag_bright?"":"bg_black"}}' style="width:{{cut_left}}px;transition-property:{{cut_animation?'':'background'}}"></view>
					<view class='content_middle_middle' style="width:{{width}}px;height:{{height}}px;transition-duration: .3s;transition-property:{{cut_animation?'':'background'}};">
					...
					<view class='content_middle_right bg_gray {{flag_bright?"":"bg_black"}}' style="transition-property:{{cut_animation?'':'background'}}"></view>
				</view>
				<view class='content_bottom bg_gray {{flag_bright?"":"bg_black"}}' style="transition-property:{{cut_animation?'':'background'}}"></view>
			</view>
			<image bindload="imageLoad" bindtouchstart="_start" bindtouchmove="_move" bindtouchend="_end" style="width:{{img_width ? img_width + 'px' : 'auto'}};height:{{img_height ? img_height + 'px' : 'auto'}};transform:translate3d({{img_left-img_width/2}}px,{{img_top-img_height/2}}px,0) scale({{scale}}) rotate({{angle}}deg);transition-duration:{{cut_animation?.4:0}}s;"
			 class='img' src='{{imgSrc}}'></image>
		</view>
		<canvas canvas-id='image-cropper' disable-scroll="true" style="width:{{canvas_width * export_scale}}px;height:{{canvas_height * export_scale}}px;left:{{canvas_left}}px;top:{{canvas_top}}px"
	...
	export default {
		...
		watch: {
			...
			limit_move(value, that) {
				if (value) {
					...
					!that.data.canvas_overflow && that._draw();
				}
			},
		...
		methods: {
			...
			setTransform(transform) {
				...
				this.data.img_top = transform.y ? this.data.img_top + transform.y : this.data.img_top;
				this.data.img_left = transform.x ? this.data.img_left + transform.x : this.data.img_left;
				//图像边缘检测,防止截取到空白
				...
				this.setData({
					...
					img_top: this.data.img_top,
					img_left: this.data.img_left
				});
			...
			setCutCenter() {
				...
				//顺序不能变
				this.setData({
					img_top: this.data.img_top - this.data.cut_top + cut_top,
					cut_top: cut_top, //截取的框上边距
					img_left: this.data.img_left - this.data.cut_left + cut_left,
			...
			imgReset() {
				this.setData({
					scale: 1,
					angle: 0,
					img_top: wx.getSystemInfoSync().windowHeight / 2,
					img_left: wx.getSystemInfoSync().windowWidth / 2,
				})
			},
			/**
			 * 加载（更换）图片
			...
			_initImageSize() {
				//处理宽高特殊单位 %>px
				if (this.data.INIT_IMGWIDTH && typeof this.data.INIT_IMGWIDTH == "string" && this.data.INIT_IMGWIDTH.indexOf("%") !=
					-1) {
				...
				if (this.data.INIT_IMGHEIGHT && typeof this.data.INIT_IMGHEIGHT == "string" && this.data.INIT_IMGHEIGHT.indexOf("%") !=
					-1) {
			...
			_imgMarginDetectionPosition(scale) {
				...
				let left = this.data.img_left;
				let top = this.data.img_top;
				...
				left = this.data.cut_left + this.data.width - img_width * scale / 2 <= left ? left : this.data.cut_left + this.data
					.width - img_width * scale / 2;
				top = this.data.cut_top + img_height * scale / 2 >= top ? top : this.data.cut_top + img_height * scale / 2;
				top = this.data.cut_top + this.data.height - img_height * scale / 2 <= top ? top : this.data.cut_top + this.data.height -
					img_height * scale / 2;
				this.setData({
					img_left: left,
					img_top: top,
			...
			//开始触摸
			_start(event) {
				this.data.flag_img_endtouch = false;
				if (event.touches.length == 1) {
					//单指拖动
					this.data.touch_img_relative[0] = {
						x: (event.touches[0].clientX - this.data.img_left),
						y: (event.touches[0].clientY - this.data.img_top)
					}
				} else {
					//双指放大
					...
					this.data.touch_img_relative = [{
						...
						x: (event.touches[1].clientX - this.data.img_left),
						y: (event.touches[1].clientY - this.data.img_top)
					}];
					this.data.hypotenuse_length = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
				}!this.data.canvas_overflow && this._draw();
			},
			_move_throttle() {
				//安卓需要节流
			...
			_move(event) {
				if (this.data.flag_img_endtouch || !this.data.MOVE_THROTTLE_FLAG) return;
				...
				if (event.touches.length == 1) {
					...
					//图像边缘检测,防止截取到空白
					this.data.img_left = left;
					this.data.img_top = top;
					...
					let width = (Math.abs(event.touches[0].clientX - event.touches[1].clientX)),
						...
						scale = this.data.scale * (hypotenuse / this.data.hypotenuse_length),
						current_deg = 0;
					scale = scale <= this.data.min_scale ? this.data.min_scale : scale;
					scale = scale >= this.data.max_scale ? this.data.max_scale : scale;
					//图像边缘检测,防止截取到空白
					...
					if (!this.data.disable_rotate) {
						...
						let first_atan_old = 180 / Math.PI * Math.atan2(this.data.touch_img_relative[0].y, this.data.touch_img_relative[
							0].x);
						let second_atan = 180 / Math.PI * Math.atan2(_touch_img_relative[1].y, _touch_img_relative[1].x);
						let second_atan_old = 180 / Math.PI * Math.atan2(this.data.touch_img_relative[1].y, this.data.touch_img_relative[
							1].x);
						//当前旋转的角度
					...
					this.data.touch_img_relative = _touch_img_relative;
					this.data.hypotenuse_length = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
					//更新视图
					this.setData({
			...
			//结束操作
			_end(event) {
				this.data.flag_img_endtouch = true;
				this._moveStop();
			},
			//点击中间剪裁框处理
			_click(event) {
				...
				this._draw(() => {
					...
					if ((x >= this.data.cut_left && x <= (this.data.cut_left + this.data.width)) && (y >= this.data.cut_top && y <=
							(this.data.cut_top + this.data.height))) {
						//生成图片并回调
						wx.canvasToTempFilePath({
							width: this.data.width * this.data.export_scale,
							height: Math.round(this.data.height * this.data.export_scale),
			...
			_draw(callback) {
				...
				let draw = () => {
					...
					//canvas和图片的相对距离
					var xpos = this.data.img_left - this.data.cut_left;
					var ypos = this.data.img_top - this.data.cut_top;
					//旋转画布
			...
			//裁剪框处理
			_cutTouchMove(e) {
				if (this.data.flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
					...
					let width = this.data.width,
						...
						size_correct = () => {
							...
							height = height <= this.data.max_height ? height >= this.data.min_height ? height : this.data.min_height : this.data
								.max_height;
						},
						size_inspect = () => {
							if ((width > this.data.max_width || width < this.data.min_width || height > this.data.max_height || height <
									this.data.min_height) && this.data.disable_ratio) {
								size_correct();
								return false;
							} else {
								size_correct();
					...
					height = this.data.CUT_START.height + ((this.data.CUT_START.corner > 1 && this.data.CUT_START.corner < 4 ? 1 : -1) *
			...
			_cutTouchEnd(e) {
				this._moveStop();
				this.data.flag_cut_touch = false;
			},
			//停止移动时需要做的操作
			_moveStop() {
				//清空之前的自动居中延迟函数并添加最新的
			...
			_observe(obj, key, watchFun) {
				...
				Object.defineProperty(obj, key, {
					...
					get() {
						if (val &&
							'img_top|img_left||width|height|min_width|max_width|min_height|max_height|export_scale|cut_top|cut_left|canvas_top|canvas_left|img_width|img_height|scale|angle|min_scale|max_scale'
							.indexOf(key) != -1) {
